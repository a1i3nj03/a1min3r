...
__host__ extern int ark_reset(int thr_id);
...
//			ark_reset(thr_id);
			...
			if (rc == -128)
			{
				if (opt_time_limit > 0)
				{
					rc = scanhash_x16r(thr_id, &work, max_nonce, &hashes_done, ~0ULL);
				}
				else
					rc = scanhash_x16r(thr_id, &work, 1 << 28, &hashes_done, opt_seq);
			}
			if (rc < 0 && (hashes_done > nonceptr[0] - max_nonce || hashes_done == 0))
				hashes_done = nonceptr[0] - max_nonce;
//				if (rc == -127)
...
//			double rate_factor = 1.0;
			...
			if (dtime > 0.0) {
				...
				pthread_mutex_unlock(&stats_lock);
//				putchar('.');
//				gpulog(LOG_INFO, thr_id, "%f", dtime);
			}
		}
...
//		else if (rc > 1)
			...
			if (nonceptr[0] > UINT32_MAX - (1 << 22)) // 64
			{
				g_work_time = 0;
