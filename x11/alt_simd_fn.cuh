...
__device__ __forceinline__
static void STEP8_IF_IV(int IV_idx, const uint32_t i, const uint32_t r, const uint32_t s, uint32_t *const __restrict__ A, const uint32_t *const __restrict__ B, const uint32_t *const __restrict__ C, uint32_t *const __restrict__ D)
{
	uint32_t R[8];
...
#elif 1
	//	*(uint2x4*)&D[0] = __ldg4((uint2x4*)&c_IV_512[IV_idx]) + *(uint2x4*)&D[0];
#pragma unroll 8
	for (int j = 0; j<8; j++)
		D[j] += c_IV_512[IV_idx + j] + IF(A[j], B[j], C[j]);
#else
#pragma unroll 8
	for (int j = 0; j<8; j++)
		D[j] += w[j] + IF(A[j], B[j], C[j]);
...
#define STEP8_IF_FINAL_1(w, i, r, s, A, B, C, D)	\
do {\
	uint32_t R[8];\
	R[0] = ROTL32((A)[0], r);\
	R[1] = ROTL32((A)[1], r);\
\
	R[0 ^ p8_xor(i)] = ROTL32((A)[0 ^ p8_xor(i)], r);\
	R[1 ^ p8_xor(i)] = ROTL32((A)[1 ^ p8_xor(i)], r);\
	(D)[0] += (w)[0] + IF((A)[0], (B)[0], (C)[0]);\
	(D)[1] += (w)[1] + IF((A)[1], (B)[1], (C)[1]);\
\
	(D)[0] = R[0 ^ p8_xor(i)] + ROTL32((D)[0], s);\
	(D)[1] = R[1 ^ p8_xor(i)] + ROTL32((D)[1], s);\
\
	(A)[0] = R[0];\
	(A)[1] = R[1];\
} while (0)

#define STEP8_IF_FINAL(w, i, r, s, A, B, C, D)	\
do {\
	uint32_t R[8];\
	R[0] = ROTL32((A)[0], r);\
	R[1] = ROTL32((A)[1], r);\
...
static void Round8(uint32_t*const __restrict__  A, const uint32_t thr_offset, const uint4 *const __restrict__ g_fft4) {
	...
	STEP8_IF4(tmp, 0, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_IF4(tmp, 1, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_IF4(tmp, 2, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_IF4(tmp, 3, u, r, &A[8], &A[16], &A[24], A);
	tmp += 2;
	STEP8_MAJ4(tmp, 4, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_MAJ4(tmp, 5, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_MAJ4(tmp, 6, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_MAJ4(tmp, 7, u, r, &A[8], &A[16], &A[24], A);

	r = 28; s = 19; t = 22; u = 7;
	tmp += 2;
	STEP8_IF4(tmp, 8, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_IF4(tmp, 9, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_IF4(tmp, 10, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_IF4(tmp, 11, u, r, &A[8], &A[16], &A[24], A);
	tmp += 2;
	STEP8_MAJ4(tmp, 12, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_MAJ4(tmp, 13, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_MAJ4(tmp, 14, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_MAJ4(tmp, 15, u, r, &A[8], &A[16], &A[24], A);

	r = 29; s = 9; t = 15; u = 5;

	tmp += 2;
	STEP8_IF4(tmp, 16, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_IF4(tmp, 17, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_IF4(tmp, 18, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_IF4(tmp, 19, u, r, &A[8], &A[16], &A[24], A);
	tmp += 2;
	STEP8_MAJ4(tmp, 20, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_MAJ4(tmp, 21, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_MAJ4(tmp, 22, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_MAJ4(tmp, 23, u, r, &A[8], &A[16], &A[24], A);

	r = 4; s = 13; t = 10; u = 25;

	tmp += 2;
	STEP8_IF4(tmp, 24, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_IF4(tmp, 25, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_IF4(tmp, 26, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_IF4(tmp, 27, u, r, &A[8], &A[16], &A[24], A);
	tmp += 2;
	STEP8_MAJ4(tmp, 28, r, s, A, &A[8], &A[16], &A[24]);
	tmp += 2;
	STEP8_MAJ4(tmp, 29, s, t, &A[24], A, &A[8], &A[16]);
	tmp += 2;
	STEP8_MAJ4(tmp, 30, t, u, &A[16], &A[24], A, &A[8]);
	tmp += 2;
	STEP8_MAJ4(tmp, 31, u, r, &A[8], &A[16], &A[24], A);
#else
	int tmp = 0 + thr_offset;
...
/***************************************************/
__global__ __launch_bounds__(TPB52_1, LSB)//9)
static void x11_simd512_gpu_expand_64(uint32_t threads, const uint32_t* __restrict__ g_hash, uint4 * __restrict__ g_temp4, volatile int *order)
{
